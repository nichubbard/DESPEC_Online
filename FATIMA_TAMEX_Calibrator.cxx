...
FATIMA_TAMEX_Calibrator::FATIMA_TAMEX_Calibrator(bool ONLINE){
...
FATIMA_TAMEX_Calibrator::~FATIMA_TAMEX_Calibrator(){
	if(ONLINE){
		for(int i = 0;i < 100;++i){
...
void FATIMA_TAMEX_Calibrator::load_Calibration_Files(){
	
	//load calibration map for used ids
	ifstream map_file("Configuration_Files/Calibration_FATIMA_TAMEX/MAP.dat");

	string line;
	
	if(map_file.fail()){
		cerr << "Could not find FATIMA_TAMEX Calibration file MAP" << endl;
		exit(0);
	}
	...
	for(int i = 0;i < iter;++i){
		...
		sprintf(filename,"Configuration_Files/Calibration_FATIMA_TAMEX/Calib_%d_%d.dat",tamex_id,ch_id);
		file.open(filename);

		if(file.fail()){
			cerr << "Could not find FATIMA_TAMEX Calibration file " << tamex_id << " " << ch_id << endl;
			exit(0);
		}
		while(file.good()){
			getline(file,line,'\n');
...
double FATIMA_TAMEX_Calibrator::get_Calibration_val(double value,int tamex_id_tmp,int ch_id_tmp){
	double return_val = 0;
...
void FATIMA_TAMEX_Calibrator::get_data(double** fine_T,UInt** ch_id,int tamex_iter,int* iterator){
...
void FATIMA_TAMEX_Calibrator::ONLINE_CALIBRATION(){

	//Root file to check histograms
	//TFile* ROOT_FILE = new TFile("Root_Trees/FATIMA_TAMEX_TREE.root","RECREATE");

	//output file stream
	ofstream cal_file;

	//MAP of used TamexIds with Channels 
	ofstream map_file("Configuration_Files/Calibration_FATIMA_TAMEX/MAP.dat");
	map_file << "#FATIMA_TAMEX Calibration map" << endl;
	...
	cout << "ONLINE CALIBRATION FOR FATIMA_TAMEX INITIALIZED" << endl;
	...
	for(int i = 0;i < 100;++i){
		for(int j = 0;j < 100;++j){
			...
			if(fired[i][j]){
				...
				sprintf(filename,"Configuration_Files/Calibration_FATIMA_TAMEX/Calib_%d_%d.dat",i,j);
