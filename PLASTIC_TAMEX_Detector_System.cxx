...
PLASTIC_TAMEX_Detector_System::PLASTIC_TAMEX_Detector_System(){
...
void PLASTIC_TAMEX_Detector_System::get_Event_data(Raw_Event* RAW){
    RAW->set_DATA_PLASTIC_TAMEX(iterator,edge_coarse,edge_fine,ch_ID_edge,coarse_T,fine_T,tamex_iter,lead_arr,tamex_id);
...
void PLASTIC_TAMEX_Detector_System::Process_MBS(int* pdata){
...
void PLASTIC_TAMEX_Detector_System::Process_TAMEX(){
...
void PLASTIC_TAMEX_Detector_System::skip_padding(){
...
void PLASTIC_TAMEX_Detector_System::get_trigger(){
...
void PLASTIC_TAMEX_Detector_System::reset_edges(){
    for(int i = 0;i < 4;++i){
        for(int j = 0;j < 100;++j){
            leading_hits[i][j] = 0;
            trailing_hits[i][j] = 0;
...
void PLASTIC_TAMEX_Detector_System::get_edges(){
...
//         }
  ...
  //  if(tamex_id[tamex_iter]==2)   cout<<"ch_ID_edge[tamex_iter][iterator[tamex_iter]]  " <<ch_ID_edge[tamex_iter][iterator[tamex_iter]] <<" tamex_id[tamex_iter] " <<tamex_id[tamex_iter] << " tamex_iter " << tamex_iter << endl;
//     if(iterator[tamex_iter]>100){  cout<<" edge_coarse[tamex_iter][iterator[tamex_iter]] " <<  edge_coarse[tamex_iter][iterator[tamex_iter]]<<" edge_fine " << edge_fine[tamex_iter][iterator[tamex_iter]] <<  " iterator" << iterator[tamex_iter] <<"  ch_ID_edge " << ch_ID_edge[tamex_iter][iterator[tamex_iter]] << endl;}
 //}
...
bool PLASTIC_TAMEX_Detector_System::no_error_reached(){
...
void PLASTIC_TAMEX_Detector_System::check_error(){
    //next word
    //pdata++;
...
void PLASTIC_TAMEX_Detector_System::check_trailer(){
    //next word
    pdata++;
...
void PLASTIC_TAMEX_Detector_System::calibrate_OFFLINE(){
    int channel_ID_tmp = 0;
    for(int i = 0;i <tamex_iter ;++i){
       // if(iterator[i]>100) break;
        for(int j = 0;j < iterator[i];++j){
           //if(j %2 ==1) channel_ID_tmp =  ch_num[i][j];
            //cout<< " channel_ID_tmp " << channel_ID_tmp <<endl;
            if(edge_coarse[i][j] != 131313) edge_fine[i][j] = PLASTIC_TAMEX_Calibration->get_Calibration_val(edge_fine[i][j],i,ch_num[i][j]);
...
int* PLASTIC_TAMEX_Detector_System::get_pdata(){return pdata;}

//---------------------------------------------------------------

bool PLASTIC_TAMEX_Detector_System::calibration_done(){return Calibration_Done;}

//---------------------------------------------------------------

void PLASTIC_TAMEX_Detector_System::get_Calib_type(){
    ...
    if(!FORCE){
        ...
        out << "#PLASTIC_TAMEX calibration type file" << endl;
