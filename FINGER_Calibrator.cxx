...
double FINGER_Calibrator::get_Calibration_val(double value,int tamex_id_tmp,int ch_id_tmp){
    ...
    double tmp,tmp2;
   // cout<< "ch_id_tmp "<< ch_id_tmp <<endl;
    for(int i = 0;i < nbins-1;++i){
    if(ch_id_tmp<16 && ch_id_tmp>-1) {
         //cout<< "2 ch_id_tmp "<< ch_id_tmp <<endl;
        tmp = Cal_arr[tamex_id_tmp][ch_id_tmp][i];
        tmp2 = Cal_arr[tamex_id_tmp][ch_id_tmp][i+2];
        //cout << "calib "<<tmp << " " << tmp2 << " " << value << " " << bins_x_arr[i]<<endl;
        if(value >= bins_x_arr[i] && value < bins_x_arr[i+2]){
            
            return_val = (tmp2 - tmp)/(bins_x_arr[i+2] - bins_x_arr[i])*(value_t - bins_x_arr[i+2]) + tmp;          
            break;
            }
        }
    }
...
void FINGER_Calibrator::get_data(double** fine_T,UInt** ch_id,int tamex_iter,int* iterator,UInt** ch_num){
