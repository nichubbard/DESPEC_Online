...
TXRSParameter::TXRSParameter() : TGo4Parameter("FRSParameter")
{
...
TXRSParameter::TXRSParameter(const char* name) : TGo4Parameter(name) { 
  //fill_raw_histos = kFALSE;
  fill_raw_histos = kTRUE;
  after_run176 = kTRUE;
  ...
  bfield[3] = 0.6180;       // !  FRS D4 Feld [Tesla]  !
                            ...
                            // ! effective   Rho1..Rho4	       !
                            // !--------------------------------!
  rho0[0] = 2.4;       // ! FRS TA/S2 Sollbahnradius [m]   !
  rho0[1] = 2.4;        // ! FRS S2/S4 Sollbahnradius [m]   !
...
TXRSParameter::~TXRSParameter() { 
}

void TXRSParameter::Print(Option_t*) const {	
...
Bool_t TXRSParameter::UpdateFrom(TGo4Parameter *pp) { 
  TXRSParameter *from = dynamic_cast<TXRSParameter*> (pp);
  if (from==0) {
...
Bool_t TMRTOFMSParameter::UpdateFrom(TGo4Parameter* par) {
  ...
  MRTOFMS_TXRS = from->MRTOFMS_TXRS;
